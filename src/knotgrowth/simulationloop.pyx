...
def simulation_loop(label_grid, num_labels, grid_shape, penalty_radius, num_iterations, sigma, connectivity_padding, mask_penalty, region_history, volume_conservation):
    ...
    for iter_num in tqdm(range(num_iterations)):
        ...
        # Compute psi fields
        psies = calc.psi_3d_optimized(current_grid, sigma, dt)
        ...
        alpha = 5.0
        epsilonBar = 1e-6
        next_grid = calc.auction_assignment_3d(psies, target_volumes, grid_shape, num_labels, epsilon0, epsilonBar, alpha)
